...
#endif
#endif
static void ATL_sJIK0x0x3TN1x1x3_a1_bX
 ...
 * lda=3, ldb=3, ldc=0, mu=1, nu=1, ku=3, pf=0
 ...
 */
{
   ...
   #undef Kb
#endif
void ATL_sJIK0x0x3TN3x3x0_a1_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=N, MB=0, NB=0, KB=3, 
 * lda=3, ldb=3, ldc=0, mu=6, nu=1, ku=3, pf=0
 * Generated by ATLAS/tune/blas/gemm/emit_mm.c (3.10.0)
 */
{
   const int Mb = (M/6)*6;
   ...
   const float *stN = B + (((Nb) << 1)+Nb);
   #define incAk 3
   const int incAm = 15, incAn = -(((Mb) << 1)+Mb);
   #define incBk 3
   const int incBm = -3, incBn = 3;
   #define incCm 6
   const int incCn = (ldc) - (Mb);
   ...
   if (k=M-Mb)
      ATL_sJIK0x0x3TN1x1x3_a1_bX(k, N, 3, alpha, ca + (((Mb) << 1)+Mb), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
