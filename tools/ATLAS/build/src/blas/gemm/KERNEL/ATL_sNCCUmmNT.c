#ifndef ATL_RESTRICT
#if defined(__STDC_VERSION__) && (__STDC_VERSION__/100 >= 1999)
   #define ATL_RESTRICT restrict
#else
   #define ATL_RESTRICT
#endif
#endif
#ifndef ATL_RESTRICT
#if defined(__STDC_VERSION__) && (__STDC_VERSION__/100 >= 1999)
   #define ATL_RESTRICT restrict
#else
   #define ATL_RESTRICT
#endif
#endif
static void ATL_sJIK0x0x0NT1x1x1_aX_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=N, TB=T, MB=0, NB=0, KB=0, 
 * lda=0, ldb=0, ldc=0, mu=1, nu=1, ku=1, pf=0
 * Generated by ATLAS/tune/blas/gemm/emit_mm.c (3.10.0)
 */
{
   #define Mb M
   #define Nb N
   #define Kb K
   const float *stM = A + (Mb);
   const float *stN = B + (Nb);
   const float BetaAlpha = beta / alpha;
   const int incAk = (lda);
   const int incAm = 1 - (Kb*lda), incAn = -(Mb);
   const int incBk = (ldb), incBm = -(Kb*ldb);
   #define incBn 1
   #define incCm 1
   const int incCn = (ldc) - (Mb);
   float *pC0=C;
   const float *pA0=A;
   const float *pB0=B;
   register int k;
   register float rA0;
   register float rB0;
   register float rC0_0;
   do /* N-loop */
   {
      do /* M-loop */
      {
         rA0 = BetaAlpha;
         rC0_0 = *pC0;
         rC0_0 *= rA0;
         for (k=0; k < K; k++) /* easy loop to unroll */
         {
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
         }
         rB0 = alpha;
         rC0_0 *= rB0;
         *pC0 = rC0_0;
         pC0 += incCm;
         pA0 += incAm;
         pB0 += incBm;
      }
      while(pA0 != stM);
      pC0 += incCn;
      pA0 += incAn;
      pB0 += incBn;
   }
   while(pB0 != stN);
}
#ifdef incAm
   #undef incAm
#endif
#ifdef incAn
   #undef incAn
#endif
#ifdef incAk
   #undef incAk
#endif
#ifdef incBm
   #undef incBm
#endif
#ifdef incBn
   #undef incBn
#endif
#ifdef incBk
   #undef incBk
#endif
#ifdef incCm
   #undef incCm
#endif
#ifdef incCn
   #undef incCn
#endif
#ifdef incCk
   #undef incCk
#endif
#ifdef Mb
   #undef Mb
#endif
#ifdef Nb
   #undef Nb
#endif
#ifdef Kb
   #undef Kb
#endif
#ifndef ATL_RESTRICT
#if defined(__STDC_VERSION__) && (__STDC_VERSION__/100 >= 1999)
   #define ATL_RESTRICT restrict
#else
   #define ATL_RESTRICT
#endif
#endif
static void ATL_sJIK0x0x0NT1x1x12_aX_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=N, TB=T, MB=0, NB=0, KB=0, 
 * lda=0, ldb=0, ldc=0, mu=1, nu=1, ku=12, pf=0
 * Generated by ATLAS/tune/blas/gemm/emit_mm.c (3.10.0)
 */
{
   #define Mb M
   #define Nb N
   #define Kb K
   const int Kloop = ((K-2)/12)*12, kr = K - Kloop - 2;
   const float *stM = A + (Mb);
   const float *stN = B + (Nb);
   const float BetaAlpha = beta / alpha;
   const int incAk = (lda);
   const int incAm = 1 - (Kb*lda), incAn = -(Mb);
   const int incBk = (ldb), incBm = -(Kb*ldb);
   #define incBn 1
   #define incAk0 incAk
   #define incBk0 incBk
   #define incCm 1
   const int incCn = (ldc) - (Mb);
   float *pC0=C;
   const float *pA0=A;
   const float *pB0=B;
   register int k;
   register float rA0;
   register float rB0;
   register float m0;
   register float rC0_0;
   if (K < 3)
   {
      ATL_sJIK0x0x0NT1x1x1_aX_bX(M, N, K, alpha, A, lda, B, ldb, beta, C, ldc);
      return;
   }
   do /* N-loop */
   {
      do /* M-loop */
      {
         rA0 = BetaAlpha;
         rC0_0 = *pC0;
         rC0_0 *= rA0;
/*
 *       Start pipeline
 */
         rA0 = *pA0;
         rB0 = *pB0;
         m0 = rA0 * rB0;
         pA0 += incAk0;
         pB0 += incBk0;
         rA0 = *pA0;
         rB0 = *pB0;

         for (k=0; k < Kloop; k += 12) /* easy loop to unroll */
         {
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
         } /* end K-loop */
         switch(kr)
         {
         case 1:
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            break;
         case 2:
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            break;
         case 3:
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            break;
         case 4:
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            break;
         case 5:
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            break;
         case 6:
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            break;
         case 7:
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            break;
         case 8:
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            break;
         case 9:
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            break;
         case 10:
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            break;
         case 11:
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            rC0_0 += m0;
            m0 = rA0 * rB0;
            pA0 += incAk;
            pB0 += incBk;
            rA0 = *pA0;
            rB0 = *pB0;
            break;
         case 0: ;
         }
/*
 *       Drain pipe on last iteration of K-loop
 */
         rC0_0 += m0;
         m0 = rA0 * rB0;
         pA0 += incAk0;
         pB0 += incBk0;
         rC0_0 += m0;
         rB0 = alpha;
         rC0_0 *= rB0;
         *pC0 = rC0_0;
         pC0 += incCm;
         pA0 += incAm;
         pB0 += incBm;
      }
      while(pA0 != stM);
      pC0 += incCn;
      pA0 += incAn;
      pB0 += incBn;
   }
   while(pB0 != stN);
}
#ifdef incAm
   #undef incAm
#endif
#ifdef incAn
   #undef incAn
#endif
#ifdef incAk
   #undef incAk
#endif
#ifdef incBm
   #undef incBm
#endif
#ifdef incBn
   #undef incBn
#endif
#ifdef incBk
   #undef incBk
#endif
#ifdef incCm
   #undef incCm
#endif
#ifdef incCn
   #undef incCn
#endif
#ifdef incCk
   #undef incCk
#endif
#ifdef Mb
   #undef Mb
#endif
#ifdef Nb
   #undef Nb
#endif
#ifdef Kb
   #undef Kb
#endif
void ATL_sJIK0x0x0NT0x0x0_aX_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=N, TB=T, MB=0, NB=0, KB=0, 
 * lda=0, ldb=0, ldc=0, mu=6, nu=1, ku=12, pf=0
 * Generated by ATLAS/tune/blas/gemm/emit_mm.c (3.10.0)
 */
{
   const int Mb = (M/6)*6;
   #define Nb N
   #define Kb K
   const int Kloop = ((K-1)/12)*12, kr = K - Kloop - 1;
   const float *ca=A, *cb=B;
   float *cc=C;
   const float *stM = A + (Mb);
   const float *stN = B + (Nb);
   const float BetaAlpha = beta / alpha;
   const int incAk = (lda);
   const int incAm = 6 - (Kb*lda), incAn = -(Mb);
   const int incBk = (ldb), incBm = -(Kb*ldb);
   #define incBn 1
   #define incAk0 incAk
   #define incBk0 incBk
   #define incCm 6
   const int incCn = (ldc) - (Mb);
   float *pC0=C;
   const float *pA0=A;
   const float *pB0=B;
   register int k;
   register float rA0, rA1, rA2, rA3, rA4, rA5;
   register float rB0;
   register float m0, m1, m2;
   register float rC0_0, rC1_0, rC2_0, rC3_0, rC4_0, rC5_0;
   if (pA0 != stM)
   {
      do /* N-loop */
      {
         do /* M-loop */
         {
            rA0 = BetaAlpha;
            rC0_0 = *pC0;
            rC0_0 *= rA0;
            rC1_0 = pC0[1];
            rC1_0 *= rA0;
            rC2_0 = pC0[2];
            rC2_0 *= rA0;
            rC3_0 = pC0[3];
            rC3_0 *= rA0;
            rC4_0 = pC0[4];
            rC4_0 *= rA0;
            rC5_0 = pC0[5];
            rC5_0 *= rA0;
/*
 *          Start pipeline
 */
            rA0 = *pA0;
            rB0 = *pB0;
            rA1 = pA0[1];
            rA2 = pA0[2];
            m0 = rA0 * rB0;
            rA3 = pA0[3];
            m1 = rA1 * rB0;
            rA4 = pA0[4];
            m2 = rA2 * rB0;
            rA5 = pA0[5];

            for (k=0; k < Kloop; k += 12) /* easy loop to unroll */
            {
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
            } /* end K-loop */
            switch(kr)
            {
            case 1:
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               break;
            case 2:
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               break;
            case 3:
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               break;
            case 4:
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               break;
            case 5:
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               break;
            case 6:
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               break;
            case 7:
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               break;
            case 8:
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               break;
            case 9:
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               break;
            case 10:
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               break;
            case 11:
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               rC0_0 += m0;
               m0 = rA3 * rB0;
               rC1_0 += m1;
               m1 = rA4 * rB0;
               rC2_0 += m2;
               m2 = rA5 * rB0;
               pA0 += incAk;
               pB0 += incBk;
               rA0 = *pA0;
               rB0 = *pB0;
               rA1 = pA0[1];
               rA2 = pA0[2];
               rA3 = pA0[3];
               rC3_0 += m0;
               m0 = rA0 * rB0;
               rA4 = pA0[4];
               rC4_0 += m1;
               m1 = rA1 * rB0;
               rA5 = pA0[5];
               rC5_0 += m2;
               m2 = rA2 * rB0;
               break;
            case 0: ;
            }
/*
 *          Drain pipe on last iteration of K-loop
 */
            rC0_0 += m0;
            m0 = rA3 * rB0;
            rC1_0 += m1;
            m1 = rA4 * rB0;
            rC2_0 += m2;
            m2 = rA5 * rB0;
            pA0 += incAk0;
            pB0 += incBk0;
            rC3_0 += m0;
            rC4_0 += m1;
            rC5_0 += m2;
            rB0 = alpha;
            rC0_0 *= rB0;
            rC1_0 *= rB0;
            rC2_0 *= rB0;
            rC3_0 *= rB0;
            rC4_0 *= rB0;
            rC5_0 *= rB0;
            *pC0 = rC0_0;
            pC0[1] = rC1_0;
            pC0[2] = rC2_0;
            pC0[3] = rC3_0;
            pC0[4] = rC4_0;
            pC0[5] = rC5_0;
            pC0 += incCm;
            pA0 += incAm;
            pB0 += incBm;
         }
         while(pA0 != stM);
         pC0 += incCn;
         pA0 += incAn;
         pB0 += incBn;
      }
      while(pB0 != stN);
   }
   if (k=M-Mb)
      ATL_sJIK0x0x0NT1x1x12_aX_bX(k, N, K, alpha, ca + (Mb), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
#ifdef incAn
   #undef incAn
#endif
#ifdef incAk
   #undef incAk
#endif
#ifdef incBm
   #undef incBm
#endif
#ifdef incBn
   #undef incBn
#endif
#ifdef incBk
   #undef incBk
#endif
#ifdef incCm
   #undef incCm
#endif
#ifdef incCn
   #undef incCn
#endif
#ifdef incCk
   #undef incCk
#endif
#ifdef Mb
   #undef Mb
#endif
#ifdef Nb
   #undef Nb
#endif
#ifdef Kb
   #undef Kb
#endif
