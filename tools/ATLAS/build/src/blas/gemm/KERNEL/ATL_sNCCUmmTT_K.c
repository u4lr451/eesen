...
#endif
#endif
static void ATL_sJIK0x0x60TT1x1x60_aX_bX
...
/*
 * matmul with TA=T, TB=T, MB=0, NB=0, KB=60, 
 ...
 */
{
   ...
   #define incAk 60
   const int incAm = ((lda) - 60), incAn = -(Mb*lda);
 ...
 */
   ...
   #undef Kb
#endif
void ATL_sJIK0x0x60TT0x0x0_aX_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=T, MB=0, NB=0, KB=60, 
 ...
 */
{
   ...
   const float BetaAlpha = beta / alpha;
   #define incAk 60
   const int incAm = ((((lda) << 2)+((lda) << 1)) - 60), incAn = -(Mb*lda);
   const int incBk = (ldb), incBm = -(60*(ldb));
   #define incBn 1
   const int incAk0 = ((incAk) / 60);
   #define incBk0 incBk
 ...
 */
   ...
   if (k=M-Mb)
      ATL_sJIK0x0x60TT1x1x60_aX_bX(k, N, 60, alpha, ca + (Mb*lda), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
