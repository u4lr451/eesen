#ifndef ATL_RESTRICT
#if defined(__STDC_VERSION__) && (__STDC_VERSION__/100 >= 1999)
   #define ATL_RESTRICT restrict
#else
   #define ATL_RESTRICT
#endif
#endif
#ifndef ATL_RESTRICT
#if defined(__STDC_VERSION__) && (__STDC_VERSION__/100 >= 1999)
   #define ATL_RESTRICT restrict
#else
   #define ATL_RESTRICT
#endif
#endif
static void ATL_cJIK0x0x52TN1x1x52_aX_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=N, MB=0, NB=0, KB=52, 
 * lda=0, ldb=0, ldc=0, mu=1, nu=1, ku=52, pf=0
 * Generated by ATLAS/tune/blas/gemm/emit_mm.c (3.10.0)
 */
{
   #define Mb M
   #define Nb N
   const float *stM = A + (((lda*Mb) << 1));
   const float *stN = B + (((ldb*Nb) << 1));
   const float BetaAlpha = beta / alpha;
   #define incAk 104
   const int incAm = ((((lda) - 52) << 1)), incAn = -(((Mb*lda) << 1));
   #define incBk 104
   const int incBm = -104, incBn = (((ldb) << 1));
   const int incAk0 = ((incAk) / 52), incBk0 = ((incBk) / 52);
   #define incCm 2
   const int incCn = (((ldc) << 1)) - (((Mb) << 1));
   float *pC0=C;
   const float *pA0=A;
   const float *pB0=B;
   register int k;
   register float rA0;
   register float rB0;
   register float m0, m1, m2, m3;
   register float rC0_0;
   do /* N-loop */
   {
      do /* M-loop */
      {
         rA0 = BetaAlpha;
         rC0_0 = *pC0;
         rC0_0 *= rA0;
/*
 *       Start pipeline
 */
         rA0 = *pA0;
         rB0 = *pB0;
         m0 = rA0 * rB0;
         rA0 = pA0[2];
         rB0 = pB0[2];
         m1 = rA0 * rB0;
         rA0 = pA0[4];
         rB0 = pB0[4];
         m2 = rA0 * rB0;
         rA0 = pA0[6];
         rB0 = pB0[6];
         m3 = rA0 * rB0;
         rA0 = pA0[8];
         rB0 = pB0[8];

/*
 *       Completely unrolled K-loop
 */
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[10];
         rB0 = pB0[10];
         rC0_0 += m1;
         m1 = rA0 * rB0;
         rA0 = pA0[12];
         rB0 = pB0[12];
         rC0_0 += m2;
         m2 = rA0 * rB0;
         rA0 = pA0[14];
         rB0 = pB0[14];
         rC0_0 += m3;
         m3 = rA0 * rB0;
         rA0 = pA0[16];
         rB0 = pB0[16];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[18];
         rB0 = pB0[18];
         rC0_0 += m1;
         m1 = rA0 * rB0;
         rA0 = pA0[20];
         rB0 = pB0[20];
         rC0_0 += m2;
         m2 = rA0 * rB0;
         rA0 = pA0[22];
         rB0 = pB0[22];
         rC0_0 += m3;
         m3 = rA0 * rB0;
         rA0 = pA0[24];
         rB0 = pB0[24];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[26];
         rB0 = pB0[26];
         rC0_0 += m1;
         m1 = rA0 * rB0;
         rA0 = pA0[28];
         rB0 = pB0[28];
         rC0_0 += m2;
         m2 = rA0 * rB0;
         rA0 = pA0[30];
         rB0 = pB0[30];
         rC0_0 += m3;
         m3 = rA0 * rB0;
         rA0 = pA0[32];
         rB0 = pB0[32];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[34];
         rB0 = pB0[34];
         rC0_0 += m1;
         m1 = rA0 * rB0;
         rA0 = pA0[36];
         rB0 = pB0[36];
         rC0_0 += m2;
         m2 = rA0 * rB0;
         rA0 = pA0[38];
         rB0 = pB0[38];
         rC0_0 += m3;
         m3 = rA0 * rB0;
         rA0 = pA0[40];
         rB0 = pB0[40];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[42];
         rB0 = pB0[42];
         rC0_0 += m1;
         m1 = rA0 * rB0;
         rA0 = pA0[44];
         rB0 = pB0[44];
         rC0_0 += m2;
         m2 = rA0 * rB0;
         rA0 = pA0[46];
         rB0 = pB0[46];
         rC0_0 += m3;
         m3 = rA0 * rB0;
         rA0 = pA0[48];
         rB0 = pB0[48];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[50];
         rB0 = pB0[50];
         rC0_0 += m1;
         m1 = rA0 * rB0;
         rA0 = pA0[52];
         rB0 = pB0[52];
         rC0_0 += m2;
         m2 = rA0 * rB0;
         rA0 = pA0[54];
         rB0 = pB0[54];
         rC0_0 += m3;
         m3 = rA0 * rB0;
         rA0 = pA0[56];
         rB0 = pB0[56];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[58];
         rB0 = pB0[58];
         rC0_0 += m1;
         m1 = rA0 * rB0;
         rA0 = pA0[60];
         rB0 = pB0[60];
         rC0_0 += m2;
         m2 = rA0 * rB0;
         rA0 = pA0[62];
         rB0 = pB0[62];
         rC0_0 += m3;
         m3 = rA0 * rB0;
         rA0 = pA0[64];
         rB0 = pB0[64];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[66];
         rB0 = pB0[66];
         rC0_0 += m1;
         m1 = rA0 * rB0;
         rA0 = pA0[68];
         rB0 = pB0[68];
         rC0_0 += m2;
         m2 = rA0 * rB0;
         rA0 = pA0[70];
         rB0 = pB0[70];
         rC0_0 += m3;
         m3 = rA0 * rB0;
         rA0 = pA0[72];
         rB0 = pB0[72];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[74];
         rB0 = pB0[74];
         rC0_0 += m1;
         m1 = rA0 * rB0;
         rA0 = pA0[76];
         rB0 = pB0[76];
         rC0_0 += m2;
         m2 = rA0 * rB0;
         rA0 = pA0[78];
         rB0 = pB0[78];
         rC0_0 += m3;
         m3 = rA0 * rB0;
         rA0 = pA0[80];
         rB0 = pB0[80];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[82];
         rB0 = pB0[82];
         rC0_0 += m1;
         m1 = rA0 * rB0;
         rA0 = pA0[84];
         rB0 = pB0[84];
         rC0_0 += m2;
         m2 = rA0 * rB0;
         rA0 = pA0[86];
         rB0 = pB0[86];
         rC0_0 += m3;
         m3 = rA0 * rB0;
         rA0 = pA0[88];
         rB0 = pB0[88];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[90];
         rB0 = pB0[90];
         rC0_0 += m1;
         m1 = rA0 * rB0;
         rA0 = pA0[92];
         rB0 = pB0[92];
         rC0_0 += m2;
         m2 = rA0 * rB0;
         rA0 = pA0[94];
         rB0 = pB0[94];
         rC0_0 += m3;
         m3 = rA0 * rB0;
         rA0 = pA0[96];
         rB0 = pB0[96];
         rC0_0 += m0;
         m0 = rA0 * rB0;
         rA0 = pA0[98];
         rB0 = pB0[98];
         rC0_0 += m1;
         m1 = rA0 * rB0;
         rA0 = pA0[100];
         rB0 = pB0[100];
         rC0_0 += m2;
         m2 = rA0 * rB0;
         rA0 = pA0[102];
         rB0 = pB0[102];
/*
 *       Drain pipe on last iteration of K-loop
 */
         rC0_0 += m3;
         m3 = rA0 * rB0;
         rC0_0 += m0;
         rC0_0 += m1;
         rC0_0 += m2;
         rC0_0 += m3;
         pA0 += incAk;
         pB0 += incBk;
         rB0 = alpha;
         rC0_0 *= rB0;
         *pC0 = rC0_0;
         pC0 += incCm;
         pA0 += incAm;
         pB0 += incBm;
      }
      while(pA0 != stM);
      pC0 += incCn;
      pA0 += incAn;
      pB0 += incBn;
   }
   while(pB0 != stN);
}
#ifdef incAm
   #undef incAm
#endif
#ifdef incAn
   #undef incAn
#endif
#ifdef incAk
   #undef incAk
#endif
#ifdef incBm
   #undef incBm
#endif
#ifdef incBn
   #undef incBn
#endif
#ifdef incBk
   #undef incBk
#endif
#ifdef incCm
   #undef incCm
#endif
#ifdef incCn
   #undef incCn
#endif
#ifdef incCk
   #undef incCk
#endif
#ifdef Mb
   #undef Mb
#endif
#ifdef Nb
   #undef Nb
#endif
#ifdef Kb
   #undef Kb
#endif
void ATL_cJIK0x0x52TN0x0x0_aX_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=N, MB=0, NB=0, KB=52, 
 * lda=0, ldb=0, ldc=0, mu=6, nu=1, ku=52, pf=0
 * Generated by ATLAS/tune/blas/gemm/emit_mm.c (3.10.0)
 */
{
   const int Mb = (M/6)*6;
   #define Nb N
   const float *ca=A, *cb=B;
   float *cc=C;
   const float *stM = A + (((lda*Mb) << 1));
   const float *stN = B + (((ldb*Nb) << 1));
   const float BetaAlpha = beta / alpha;
   #define incAk 104
   const int incAm = ((((((lda) << 2)+((lda) << 1)) - 52) << 1)), incAn = -(((Mb*lda) << 1));
   #define incBk 104
   const int incBm = -104, incBn = (((ldb) << 1));
   const int incAk0 = ((incAk) / 52), incBk0 = ((incBk) / 52);
   #define incCm 12
   const int incCn = (((ldc) << 1)) - (((Mb) << 1));
   float *pC0=C;
   const float *pA0=A, *pA1=pA0+(((lda) << 1)), *pA2=pA1+(((lda) << 1)), *pA3=pA2+(((lda) << 1)), *pA4=pA3+(((lda) << 1)), *pA5=pA4+(((lda) << 1));
   const float *pB0=B;
   register int k;
   register float rA0, rA1, rA2, rA3, rA4, rA5;
   register float rB0;
   register float m0, m1, m2, m3;
   register float rC0_0, rC1_0, rC2_0, rC3_0, rC4_0, rC5_0;
   if (pA0 != stM)
   {
      do /* N-loop */
      {
         do /* M-loop */
         {
            rA0 = BetaAlpha;
            rC0_0 = *pC0;
            rC0_0 *= rA0;
            rC1_0 = pC0[2];
            rC1_0 *= rA0;
            rC2_0 = pC0[4];
            rC2_0 *= rA0;
            rC3_0 = pC0[6];
            rC3_0 *= rA0;
            rC4_0 = pC0[8];
            rC4_0 *= rA0;
            rC5_0 = pC0[10];
            rC5_0 *= rA0;
/*
 *          Start pipeline
 */
            rA0 = *pA0;
            rB0 = *pB0;
            rA1 = *pA1;
            rA2 = *pA2;
            m0 = rA0 * rB0;
            rA3 = *pA3;
            m1 = rA1 * rB0;
            rA4 = *pA4;
            m2 = rA2 * rB0;
            rA5 = *pA5;
            m3 = rA3 * rB0;

/*
 *          Completely unrolled K-loop
 */
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[2];
            rB0 = pB0[2];
            rA1 = pA1[2];
            rA2 = pA2[2];
            rA3 = pA3[2];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[2];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[2];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[4];
            rB0 = pB0[4];
            rA1 = pA1[4];
            rA2 = pA2[4];
            rA3 = pA3[4];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[4];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[4];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[6];
            rB0 = pB0[6];
            rA1 = pA1[6];
            rA2 = pA2[6];
            rA3 = pA3[6];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[6];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[6];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[8];
            rB0 = pB0[8];
            rA1 = pA1[8];
            rA2 = pA2[8];
            rA3 = pA3[8];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[8];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[8];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[10];
            rB0 = pB0[10];
            rA1 = pA1[10];
            rA2 = pA2[10];
            rA3 = pA3[10];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[10];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[10];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[12];
            rB0 = pB0[12];
            rA1 = pA1[12];
            rA2 = pA2[12];
            rA3 = pA3[12];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[12];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[12];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[14];
            rB0 = pB0[14];
            rA1 = pA1[14];
            rA2 = pA2[14];
            rA3 = pA3[14];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[14];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[14];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[16];
            rB0 = pB0[16];
            rA1 = pA1[16];
            rA2 = pA2[16];
            rA3 = pA3[16];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[16];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[16];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[18];
            rB0 = pB0[18];
            rA1 = pA1[18];
            rA2 = pA2[18];
            rA3 = pA3[18];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[18];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[18];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[20];
            rB0 = pB0[20];
            rA1 = pA1[20];
            rA2 = pA2[20];
            rA3 = pA3[20];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[20];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[20];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[22];
            rB0 = pB0[22];
            rA1 = pA1[22];
            rA2 = pA2[22];
            rA3 = pA3[22];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[22];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[22];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[24];
            rB0 = pB0[24];
            rA1 = pA1[24];
            rA2 = pA2[24];
            rA3 = pA3[24];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[24];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[24];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[26];
            rB0 = pB0[26];
            rA1 = pA1[26];
            rA2 = pA2[26];
            rA3 = pA3[26];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[26];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[26];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[28];
            rB0 = pB0[28];
            rA1 = pA1[28];
            rA2 = pA2[28];
            rA3 = pA3[28];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[28];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[28];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[30];
            rB0 = pB0[30];
            rA1 = pA1[30];
            rA2 = pA2[30];
            rA3 = pA3[30];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[30];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[30];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[32];
            rB0 = pB0[32];
            rA1 = pA1[32];
            rA2 = pA2[32];
            rA3 = pA3[32];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[32];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[32];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[34];
            rB0 = pB0[34];
            rA1 = pA1[34];
            rA2 = pA2[34];
            rA3 = pA3[34];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[34];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[34];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[36];
            rB0 = pB0[36];
            rA1 = pA1[36];
            rA2 = pA2[36];
            rA3 = pA3[36];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[36];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[36];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[38];
            rB0 = pB0[38];
            rA1 = pA1[38];
            rA2 = pA2[38];
            rA3 = pA3[38];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[38];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[38];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[40];
            rB0 = pB0[40];
            rA1 = pA1[40];
            rA2 = pA2[40];
            rA3 = pA3[40];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[40];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[40];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[42];
            rB0 = pB0[42];
            rA1 = pA1[42];
            rA2 = pA2[42];
            rA3 = pA3[42];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[42];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[42];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[44];
            rB0 = pB0[44];
            rA1 = pA1[44];
            rA2 = pA2[44];
            rA3 = pA3[44];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[44];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[44];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[46];
            rB0 = pB0[46];
            rA1 = pA1[46];
            rA2 = pA2[46];
            rA3 = pA3[46];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[46];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[46];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[48];
            rB0 = pB0[48];
            rA1 = pA1[48];
            rA2 = pA2[48];
            rA3 = pA3[48];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[48];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[48];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[50];
            rB0 = pB0[50];
            rA1 = pA1[50];
            rA2 = pA2[50];
            rA3 = pA3[50];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[50];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[50];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[52];
            rB0 = pB0[52];
            rA1 = pA1[52];
            rA2 = pA2[52];
            rA3 = pA3[52];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[52];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[52];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[54];
            rB0 = pB0[54];
            rA1 = pA1[54];
            rA2 = pA2[54];
            rA3 = pA3[54];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[54];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[54];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[56];
            rB0 = pB0[56];
            rA1 = pA1[56];
            rA2 = pA2[56];
            rA3 = pA3[56];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[56];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[56];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[58];
            rB0 = pB0[58];
            rA1 = pA1[58];
            rA2 = pA2[58];
            rA3 = pA3[58];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[58];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[58];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[60];
            rB0 = pB0[60];
            rA1 = pA1[60];
            rA2 = pA2[60];
            rA3 = pA3[60];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[60];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[60];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[62];
            rB0 = pB0[62];
            rA1 = pA1[62];
            rA2 = pA2[62];
            rA3 = pA3[62];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[62];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[62];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[64];
            rB0 = pB0[64];
            rA1 = pA1[64];
            rA2 = pA2[64];
            rA3 = pA3[64];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[64];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[64];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[66];
            rB0 = pB0[66];
            rA1 = pA1[66];
            rA2 = pA2[66];
            rA3 = pA3[66];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[66];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[66];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[68];
            rB0 = pB0[68];
            rA1 = pA1[68];
            rA2 = pA2[68];
            rA3 = pA3[68];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[68];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[68];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[70];
            rB0 = pB0[70];
            rA1 = pA1[70];
            rA2 = pA2[70];
            rA3 = pA3[70];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[70];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[70];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[72];
            rB0 = pB0[72];
            rA1 = pA1[72];
            rA2 = pA2[72];
            rA3 = pA3[72];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[72];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[72];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[74];
            rB0 = pB0[74];
            rA1 = pA1[74];
            rA2 = pA2[74];
            rA3 = pA3[74];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[74];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[74];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[76];
            rB0 = pB0[76];
            rA1 = pA1[76];
            rA2 = pA2[76];
            rA3 = pA3[76];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[76];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[76];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[78];
            rB0 = pB0[78];
            rA1 = pA1[78];
            rA2 = pA2[78];
            rA3 = pA3[78];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[78];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[78];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[80];
            rB0 = pB0[80];
            rA1 = pA1[80];
            rA2 = pA2[80];
            rA3 = pA3[80];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[80];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[80];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[82];
            rB0 = pB0[82];
            rA1 = pA1[82];
            rA2 = pA2[82];
            rA3 = pA3[82];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[82];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[82];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[84];
            rB0 = pB0[84];
            rA1 = pA1[84];
            rA2 = pA2[84];
            rA3 = pA3[84];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[84];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[84];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[86];
            rB0 = pB0[86];
            rA1 = pA1[86];
            rA2 = pA2[86];
            rA3 = pA3[86];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[86];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[86];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[88];
            rB0 = pB0[88];
            rA1 = pA1[88];
            rA2 = pA2[88];
            rA3 = pA3[88];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[88];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[88];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[90];
            rB0 = pB0[90];
            rA1 = pA1[90];
            rA2 = pA2[90];
            rA3 = pA3[90];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[90];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[90];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[92];
            rB0 = pB0[92];
            rA1 = pA1[92];
            rA2 = pA2[92];
            rA3 = pA3[92];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[92];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[92];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[94];
            rB0 = pB0[94];
            rA1 = pA1[94];
            rA2 = pA2[94];
            rA3 = pA3[94];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[94];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[94];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[96];
            rB0 = pB0[96];
            rA1 = pA1[96];
            rA2 = pA2[96];
            rA3 = pA3[96];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[96];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[96];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[98];
            rB0 = pB0[98];
            rA1 = pA1[98];
            rA2 = pA2[98];
            rA3 = pA3[98];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[98];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[98];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rA0 = pA0[100];
            rB0 = pB0[100];
            rA1 = pA1[100];
            rA2 = pA2[100];
            rA3 = pA3[100];
            rC2_0 += m0;
            m0 = rA0 * rB0;
            rA4 = pA4[100];
            rC3_0 += m1;
            m1 = rA1 * rB0;
            rA5 = pA5[100];
            rC4_0 += m2;
            m2 = rA2 * rB0;
            rC5_0 += m3;
            m3 = rA3 * rB0;
            rC0_0 += m0;
            m0 = rA4 * rB0;
            rC1_0 += m1;
            m1 = rA5 * rB0;
            rA0 = pA0[102];
            rB0 = pB0[102];
            rA1 = pA1[102];
            rA2 = pA2[102];
            rA3 = pA3[102];
            rC2_0 += m2;
            m2 = rA0 * rB0;
            rA4 = pA4[102];
            rC3_0 += m3;
            m3 = rA1 * rB0;
            rA5 = pA5[102];
            rC4_0 += m0;
            m0 = rA2 * rB0;
            rC5_0 += m1;
            m1 = rA3 * rB0;
/*
 *          Drain pipe on last iteration of K-loop
 */
            rC0_0 += m2;
            m2 = rA4 * rB0;
            rC1_0 += m3;
            m3 = rA5 * rB0;
            rC2_0 += m0;
            rC3_0 += m1;
            rC4_0 += m2;
            rC5_0 += m3;
            pA0 += incAk;
            pA1 += incAk;
            pA2 += incAk;
            pA3 += incAk;
            pA4 += incAk;
            pA5 += incAk;
            pB0 += incBk;
            rB0 = alpha;
            rC0_0 *= rB0;
            rC1_0 *= rB0;
            rC2_0 *= rB0;
            rC3_0 *= rB0;
            rC4_0 *= rB0;
            rC5_0 *= rB0;
            *pC0 = rC0_0;
            pC0[2] = rC1_0;
            pC0[4] = rC2_0;
            pC0[6] = rC3_0;
            pC0[8] = rC4_0;
            pC0[10] = rC5_0;
            pC0 += incCm;
            pA0 += incAm;
            pA1 += incAm;
            pA2 += incAm;
            pA3 += incAm;
            pA4 += incAm;
            pA5 += incAm;
            pB0 += incBm;
         }
         while(pA0 != stM);
         pC0 += incCn;
         pA0 += incAn;
         pA1 += incAn;
         pA2 += incAn;
         pA3 += incAn;
         pA4 += incAn;
         pA5 += incAn;
         pB0 += incBn;
      }
      while(pB0 != stN);
   }
   if (k=M-Mb)
      ATL_cJIK0x0x52TN1x1x52_aX_bX(k, N, 52, alpha, ca + (((Mb*lda) << 1)), lda, cb, ldb, beta, cc + (((Mb) << 1)), ldc);
}
#ifdef incAm
   #undef incAm
#endif
#ifdef incAn
   #undef incAn
#endif
#ifdef incAk
   #undef incAk
#endif
#ifdef incBm
   #undef incBm
#endif
#ifdef incBn
   #undef incBn
#endif
#ifdef incBk
   #undef incBk
#endif
#ifdef incCm
   #undef incCm
#endif
#ifdef incCn
   #undef incCn
#endif
#ifdef incCk
   #undef incCk
#endif
#ifdef Mb
   #undef Mb
#endif
#ifdef Nb
   #undef Nb
#endif
#ifdef Kb
   #undef Kb
#endif
